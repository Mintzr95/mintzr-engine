...
extern "C" __attribute__((visibility("default"))) void mju_android_start(int w,int h){
 ...
 g_renderer.initialize();g_renderer.resize(w,h);
}
extern "C" __attribute__((visibility("default"))) void mju_android_resize(int w,int h){g_app.resize(w,h);g_renderer.resize(w,h);}
extern "C" __attribute__((visibility("default"))) void mju_android_tick(float dt){g_app.tick(dt);g_renderer.begin();g_renderer.draw(g_app.engine().scene());g_renderer.end();}
