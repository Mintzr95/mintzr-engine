...
int main() {
    ...
    child.transform.position = {10, 5};
    root.transform.position = {100, 50};
    root.transform.scale = {2, 2};
    root.transform.rotation = 90;
    ...
    Scene physics_scene;
    auto& a = physics_scene.create_entity("a");
    auto& b = physics_scene.create_entity("b");
    a.transform.position = {0, 0};
    b.transform.position = {40, 0};
    a.sprite.size = {50, 50};
    b.sprite.size = {50, 50};

    physics::World world_physics;
    auto& ba = world_physics.add_body(a.id, physics::BodyType::Dynamic);
    ba.useGravity = false;
    ba.restitution = 0.5f;
    ...
    cfg.maxParticles = 32;
    cfg.emissionRate = 100;
    cfg.lifetime = 0.05f;
    emitter.configure(cfg);
    emitter.start();
    emitter.update(0.02f, {0, 0});
    const auto first_capacity = emitter.particles().capacity();
    assert(!emitter.particles().empty());
    emitter.update(0.2f, {0, 0});
    assert(emitter.particles().capacity() == first_capacity);
    assert(emitter.particles().size() <= 32);
    return 0;
}
