...
bool scene_from_json(Scene&scene,const std::string&text){
 try{
  const json root=json::parse(text);if(root.value("format","")!="mju-scene")return false;const int version=root.value("version",1);if(version<1||version>3||!root.contains("entities")||!root["entities"].is_array())return false;
  if(root.contains("checksum")&&root["checksum"].is_number_unsigned()&&root["checksum"].get<std::uint64_t>()!=fnv1a(canonical_without_checksum(root)))return false;
  scene.clear();std::vector<EntityId>ids,parents;std::unordered_set<EntityId>seen;EntityId max_id=1;
  for(const auto&v:root["entities"]){const EntityId id=v.value("id",0u);if(!id||!seen.insert(id).second)return false;auto*e=scene.create_entity_with_id(v.value("name","Entity"),id,0);if(!e)return false;ids.push_back(id);parents.push_back(v.value("parent",0u));auto p=v.value("position",std::vector<float>{0,0}),s=v.value("scale",std::vector<float>{1,1}),c=v.value("color",std::vector<float>{1,1,1,1}),sz=v.value("size",std::vector<float>{96,96});if(p.size()>=2)e->transform.position={p[0],p[1]};if(s.size()>=2)e->transform.scale={s[0],s[1]};e->transform.rotation=v.value("rotation",0.0f);if(c.size()>=4)e->sprite.color={c[0],c[1],c[2],c[3]};if(sz.size()>=2)e->sprite.size={sz[0],sz[1]};e->sprite.visible=v.value("visible",true);e->sprite.texture=v.value("texture",std::string{});auto u0=v.value("uv_min",std::vector<float>{0,0}),u1=v.value("uv_max",std::vector<float>{1,1});if(u0.size()>=2)e->sprite.uv_min={u0[0],u0[1]};if(u1.size()>=2)e->sprite.uv_max={u1[0],u1[1]};e->sprite.frame_width=std::max(0,v.value("frame_width",0));e->sprite.frame_height=std::max(0,v.value("frame_height",0));e->sprite.frame=std::max(0,v.value("frame",0));e->sprite.frame_count=std::max(1,v.value("frame_count",1));e->sprite.fps=std::max(0.0f,v.value("fps",0.0f));e->sprite.animation_time=std::max(0.0f,v.value("animation_time",0.0f));e->sprite.animation_loop=v.value("animation_loop",true);e->sprite.animation_playing=v.value("animation_playing",false);e->layer=v.value("layer",0);e->active=v.value("active",true);e->visible=v.value("entity_visible",true);e->locked=v.value("locked",false);if(id==std::numeric_limits<EntityId>::max())max_id=id;else max_id=std::max(max_id,static_cast<EntityId>(id+1));}
