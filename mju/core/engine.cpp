...
bool Engine::initialize(int w,int h){
 if(w<=0||h<=0)return false;width_=w;height_=h;frame_count_=0;demo_entity_=button_entity_=0;scene_.clear();camera_={};audio_.initialize();physics_.clear();particles_.clear();resources_.clear();ui_.clear();
 ...
 auto&root=scene_.create_entity("MJU Demo");demo_entity_=root.id;root.transform.position={w*0.5f,h*0.5f};root.sprite.size={140,140};root.sprite.color={0.15f,0.75f,1.0f,1.0f};
 auto&button=scene_.create_entity("Touch Button");button_entity_=button.id;button.transform.position={100.0f,h-100.0f};button.sprite.size={160,80};button.sprite.color={0.25f,0.5f,0.95f,1.0f};console_.info("MJU Engine initialized");initialized_=true;return true;
}
void Engine::resize(int w,int h){if(w>0&&h>0){width_=w;height_=h;ui_.layout((float)w,(float)h);}}
void Engine::update(float dt){
 if(!initialized_)return;dt=std::clamp(dt,0.0f,0.1f);
 for(auto&e:scene_.entities()){
  if(e.id==demo_entity_)e.transform.rotation+=dt*30.0f;
  auto&s=e.sprite;
 ...
 auto*root=scene_.find(demo_entity_);const auto&t=input_.primary();auto*button=scene_.find(button_entity_);
...
void Engine::shutdown(){audio_.shutdown();physics_.clear();particles_.clear();resources_.clear();ui_.clear();scene_.clear();demo_entity_=button_entity_=0;console_.info("MJU Engine shutdown");initialized_=false;}
}
