...
bool Engine::initialize(int w,int h){
 ...
 width_=w;height_=h;frame_count_=0;scene_.clear();camera_={};audio_.initialize(); physics_.clear(); particles_.clear(); resources_.clear(); ui_.clear(); ui::Widget& play = ui_.add("play", "PLAY"); play.rect.size={180,72}; play.anchor=ui::Anchor::Bottom; play.on_click=[this](){ camera_.zoom = camera_.zoom > 0.75f ? 0.75f : 1.0f; }; ui_.layout((float)w,(float)h); particles::EmitterConfig pc; pc.maxParticles=256; pc.emissionRate=20; pc.lifetime=0.8f; pc.speed=55; pc.size=7; pc.startColor={0.25f,0.8f,1,1}; particles_.configure(pc); particles_.start();
 auto& root=scene_.create_entity("MJU Demo");
 root.transform.position={w*0.5f,h*0.5f};
 root.sprite.size={140,140};root.sprite.color={0.15f,0.75f,1.0f,1.0f};
 auto& button=scene_.create_entity("Touch Button");
 button.transform.position={100.0f,h-100.0f};button.sprite.size={160,80};button.sprite.color={0.25f,0.5f,0.95f,1.0f};
...
void Engine::update(float dt){
 ...
 for(auto&e:scene_.entities()) if(e.name=="MJU Demo") e.transform.rotation += dt*30.0f;
 auto* root = scene_.find(1); if(root) physics_.add_body(root->id,physics::BodyType::Kinematic);
 const auto&t=input_.primary();
 auto*button=scene_.find(2);
