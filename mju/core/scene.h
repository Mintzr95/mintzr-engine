...
struct Sprite { Color color{}; Vec2 size{96,96}; bool visible=true; std::string texture; Vec2 uv_min{0,0}; Vec2 uv_max{1,1}; int frame_width=0; int frame_height=0; int frame=0; int frame_count=1; float fps=0.0f; float animation_time=0.0f; bool animation_loop=true; bool animation_playing=false; };
struct Entity { EntityId id=0; EntityId parent=0; std::string name; Transform2D transform{}; Sprite sprite{}; int layer=0; bool active=true; bool visible=true; bool locked=false; };

class Scene {
public:
    explicit Scene(std::size_t max_entities=65536);
    Entity& create_entity(std::string name, EntityId parent=0);
    Entity* create_entity_with_id(std::string name, EntityId id, EntityId parent=0);
    bool destroy_entity(EntityId id, bool destroy_children=true);
    Entity* duplicate_entity(EntityId id, EntityId parent_override=0);
    Entity* find(EntityId id); const Entity* find(EntityId id) const;
    ...
    void clear(); void reset_id_counter(EntityId next_id);
