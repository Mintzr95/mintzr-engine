...
public:
 ...
 std::int32_t width() const{return width_;} std::int32_t height() const{return height_;}
 ...
 std::vector<Rect2> visible_cells(const Rect2& world,float cell_size) const;
private: bool inside(std::int32_t x,std::int32_t y) const; std::int32_t width_=0,height_=0; std::vector<Tile> tiles_;
};
}
